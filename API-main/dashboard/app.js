...
// --- CAGR CHART ---
  state.cagrChart = buildBarChart(
    ...
    "CAGR",
    labels,
    validFunds.map((fund) => fund.cagr ?? 0),
    validFunds.map((fund) => {
        if ((fund.cagr ?? -1) > 0.15) { // Más de 15% (Excelente)
            return "rgba(57, 201, 128, 0.8)";
        }
        if ((fund.cagr ?? -1) >= 0) { // Entre 0% y 15% (Aceptable)
            return "rgba(244, 195, 93, 0.8)";
        }
        return "rgba(255, 107, 107, 0.8)"; // Negativo (Malo)
    })
  );
...
// --- MAX DRAWDOWN CHART ---
  state.maxDrawdownChart = buildBarChart(
    ...
    "Max Drawdown",
    labels,
    validFunds.map((fund) => fund.max_drawdown ?? 0),
    validFunds.map((fund) => {
        if ((fund.max_drawdown ?? -1) > -0.10) { // Caída menor al 10% (Muy seguro)
            return "rgba(57, 201, 128, 0.8)";
        }
        if ((fund.max_drawdown ?? -1) > -0.25) { // Caída entre 10% y 25% (Moderado)
            return "rgba(244, 195, 93, 0.8)";
        }
        return "rgba(255, 107, 107, 0.8)"; // Caída mayor al 25% (Riesgoso)
    })
  );
...
// --- SORTINO CHART ---
  state.sortinoChart = buildBarChart(
    ...
    "Sortino Ratio",
    labels,
    validFunds.map((fund) => fund.sortino ?? 0),
    validFunds.map((fund) => {
        if ((fund.sortino ?? -1) > 1) {
            return "rgba(57, 201, 128, 0.8)";
        }
...
// --- VOLATILITY CHART ---
  state.volatilityChart = buildBarChart(
    ...
    "Volatility",
    labels,
    validFunds.map((fund) => fund.volatility ?? 0),
    validFunds.map((fund) => {
        if ((fund.volatility ?? -1) < 0.10) { // Volatilidad baja (Estable)
            return "rgba(57, 201, 128, 0.8)";
        }
        if ((fund.volatility ?? -1) <= 0.20) { // Volatilidad media
            return "rgba(244, 195, 93, 0.8)";
        }
        return "rgba(255, 107, 107, 0.8)"; // Volatilidad alta
    })
  );
...
// --- RSI CHART ---
  state.rsiChart = buildBarChart(
    ...
    "RSI",
    labels,
    validFunds.map((fund) => fund.rsi ?? 50),
    validFunds.map((fund) => {
        // En RSI, lo ideal suele ser zona neutral (entre 30 y 70)
        if ((fund.rsi ?? 50) >= 30 && (fund.rsi ?? 50) <= 70) {
            return "rgba(57, 201, 128, 0.8)"; // Zona neutral / saludable
        }
        if ((fund.rsi ?? 50) > 70) {
            return "rgba(255, 107, 107, 0.8)"; // Sobrecompra (Riesgo de caída)
        }
        return "rgba(244, 195, 93, 0.8)"; // Sobreventa (Oportunidad o peligro)
    })
  );
}
